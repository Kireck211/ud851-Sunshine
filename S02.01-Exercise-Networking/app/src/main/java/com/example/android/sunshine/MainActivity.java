...
public class MainActivity extends AppCompatActivity {
    ...
    private void makeOpenWeatherQuery() {
        new OpenWeatherTask().execute(NetworkUtils.buildUrl(""));
    }

    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    class OpenWeatherTask extends AsyncTask<URL, Void, String> {

        @Override
        protected String doInBackground(URL... urls) {
            URL searchUrl = urls[0];
            String OpenWeatherResults = null;
            try {
                OpenWeatherResults = NetworkUtils.getResponseFromHttpUrl(searchUrl);
            } catch (IOException e) {
                e.printStackTrace();
            }
